...
def main(args):
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            '''
            #the following line runs a forward pass of the model, using as input the source tokens, their lengths, and the target input tokens
            output, _ = model(sample['src_tokens'], sample['src_lengths'], sample['tgt_inputs'])
            #output.size = [batch_soze, tgt_seq_len, vocab_size]
            #the output represents a distribution over the vocabulary for each token in the target sequence

            #the following lines compute the loss between the model's predictions and the true, target tokens, using cross-entropy
            loss = \
                criterion(output.view(-1, output.size(-1)), sample['tgt_tokens'].view(-1)) / len(sample['src_lengths'])   #the predictions are reshaped into a 2D tensor with shape [-1, vocabulary_size] to align with the flattened target tokens, and the loss is normalised by dividing by the length of the source lengths
           #the following lines perform backpropagation to compute the gradients of the loss w.r.t the model's parameters
            loss.backward()
            #Gradients are clipped to prevent the exploding gradient problem
            grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip_norm)
            #the model parameters are updated based on the computed gradients from backprop
            optimizer.step()
            #gradients of all parameters are reset to zero to prevent them from accummulating
            optimizer.zero_grad()
