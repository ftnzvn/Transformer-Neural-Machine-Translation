...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        We need to apply a mask to prevent the model from attending to padding tokens within the input sequence.
        Padding is added to ensure sentences in a batch are of the same length, but we apply the mask
        and set the attention scores to '-inf' before the softmax operation, so as not to attend to the
        padding when computing the context vector. This ensures that the model's attention mechanism
        focuses only on relevant parts of the input sequence.
        '''
        if src_mask is not None:
            src_mask = src_mask.unsqueeze(dim=1)  #src_mask.size = [batch_size, 1, src_time_steps]
            attn_scores.masked_fill_(src_mask, float('-inf'))  #attn_scores.masked_fill.size = [batch_size, 1, src_time_steps]

        attn_weights = F.softmax(attn_scores, dim=-1)  #attn_weights.size = [batch_size, 1, src_time_steps]
        attn_context = torch.bmm(attn_weights, encoder_out).squeeze(dim=1)  #attn_context.size = [batch_size, output_dims]

        context_plus_hidden = torch.cat([tgt_input, attn_context], dim=1)  #context_plus_hidden.size = [batch_size, input_dims + output dims]
        attn_out = torch.tanh(self.context_plus_hidden_projection(context_plus_hidden))  #attn_out.size = [batch_size, output_dims]
    ...
    def score(self, tgt_input, encoder_out):
        ...
        Attention scores are calculated sentence by sentence, by taking the dot product of
        the current decoder hidden state and the linearly transformed encoder hidden state for each timestep of the input.
        The dot product is carried out for all sentences in the batch using the 'torch.bmm' function.
        These attention scores reflect the alignment between the query vector from the decoder and the key vectors from the encoder.
        The scores are then used to create a context vector that collects information across the sequence,emphasising the most relevant parts for the task at hand.
        This is particularly useful in the task of NMT, where not all words in the input sentence contribute equally to the meaning of each word in the translated sentence.
        '''
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            # TODO: --------------------------------------------------------------------- CUT
            self.context_hid = nn.Linear(embed_dim, embed_dim, False)
            self.final_context_projection = nn.Linear(embed_dim, len(dictionary))
            # TODO: --------------------------------------------------------------------- /CUT
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        cached_state == None when there is no cached state to pick up from or when  incremental_state == None. 
        In other words, this occurs when there are no previous hidden or cell states available to retrieve.
        This can happen at the beginning of the decoding or when the model begins to process a new input sequence,
        or if the generation is non-incremental (the model processes the entire input sequence in a single forward pass, not in a token-by-token fashion)
        
        2.  What role does input_feed play?
        'input_feed' is initialised as a zero tensor when there's no cached state available,
        indicating the start of a sequence or the absence of incremental state. As the sequence generation
        progresses, it is updated with context information from the previous timestep or with the previous hidden decoder state,
        "remembering" attended contextual information. This, in turn, improves the model's ability to capture long-range dependencies and relationships within the input sequence.
        It acts as an additional memory loop, ensuring no information is lost over long sequences from vanishing or exploding gradients. 
        The effect of this on a NMT task, is that it helps generate contextually relevant outputs and capture dependencies 
        in the input or output language such as subject-verb relations. Especially given a language pair like English-German,
        this is important because German has long-range dependencies whereby the verb has to go to the end of the sentence.
        By ensuring the model remembers that the verb bears a relation to the subject, we can ensure better translation quality. 
        '''
        ...
        for j in range(tgt_time_steps):
            ...
            This is done so that the attention function focuses on different parts of the input
            sequence for each step of the output generation. The previous target state contains important contextual information,
            which is useful for a task such as machine translation, where context is crucial for 
            determining meaning and relevance/dependencies of words. This helps calculate attention scores more accurately.
            Using the previous target state as input also improves the alignment between the input and the output, allowing the 
            model to generate accurate and contextually relevant sequences.
            
            2.  What is the purpose of the dropout layer?
            
            The dropout layer is used for regularisation, to reduce co-adaptations in the training data
            and, in extension, to prevent overfitting. It adds noise and reduces correlations (or co-adaptations) between hidden units,
            forcing the model to rely less on training data, ensuring that the LSTM doesn't just memorise training sentences,
            forms better generalisations, and handles variation in the input data more robustly. 
            '''
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    # TODO: --------------------------------------------------------------------- CUT
                    avg_embedding = torch.tanh(torch.bmm(step_attn_weights.unsqueeze(dim=1), src_embeddings.transpose(0, 1)).squeeze(dim=1))
                    context_vec = torch.tanh(self.context_hid(avg_embedding)) + avg_embedding
                    lexical_contexts.append(context_vec)
                    # TODO: --------------------------------------------------------------------- /CUT
        ...
        if self.use_lexical_model:
            ...
            lexical_contexts_output = self.final_context_projection(lexical_contexts)
            decoder_output = decoder_output + lexical_contexts_output
            # TODO: --------------------------------------------------------------------- /CUT
